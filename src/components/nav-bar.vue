    ...
    >
      ...
      <nav>
        <ul>
          ...
          <li><nuxt-link class="nav-link" to="#product">PRODUCTS</nuxt-link></li>
...
export default {
  ...
  beforeUnmount() {
    window.removeEventListener('scroll', this.handleScroll); // Important to prevent memory leaks
  },
  methods: {
    handleScroll() {
  ...
  *::after {
  ...
  .nav-container {
    ...
    position: fixed; /* or fixed if you want it to remain on scroll */
    top: 0;
    left: 0;
    width: 100%;
    z-index: 10;
  ...
  .nav-container.scrolled-nav {
    background-color: rgba(0, 0, 0, 0.5); /* Slightly black transparent background */
  }
